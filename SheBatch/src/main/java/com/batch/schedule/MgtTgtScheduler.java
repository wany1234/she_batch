...
@Service
public class MgtTgtScheduler extends BaseScheduler {
    @Autowired
    ...
    private static final Logger logger = LoggerFactory.getLogger(MgtTgtScheduler.class);

    public MgtTgtScheduler(EduMasterService eduMasterService) {
        this.batchCd = "TGT_DEPT_LMT_DT_SERVICE";
    }
    ...
    public void run() throws Exception {
        try {
            ...
            // 4. 말일에서 10일 또는 배치 서비스에 설정된 배치기준일수를 더한 날짜 계산
            ...
            String message = this.mgtTgtService.getMgtTgtList(date, batchDay);
